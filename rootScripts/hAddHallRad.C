/// update from $ROOTSYS/tutorial/io/hadd.C
#include <string.h>
#include "TChain.h"
#include "TFile.h"
#include "TH1.h"
...
void AverageAdd(TH1 *h1, TH1 *h2);

void hadd(string list) {
   ...
   string data;
   while(ifile>>data)
     TList->Add( TFile::Open(data.c_str()) );
...
void MergeRootfile( TDirectory *target, TList *sourcelist ) {
   ...
   while ( (key = (TKey*)nextkey())) {
      ...
      if ( obj->IsA()->InheritsFrom( TH1::Class() ) ) {
         ...
         TH1 *h1 = (TH1*)obj;
	 string hNm = h1->GetName();
	 int avgHist(0);
	 if( hNm.find("ha") < hNm.size() )
	   avgHist=1;
...
void AverageAdd(TH1 *h1, TH1 *h2){
  ...
  for(int i=1;i<=nbins;i++){
    ...
    double d1=h1->GetBinError(i);
    double v2=h2->GetBinContent(i);
    double d2=h2->GetBinError(i);

    h1->SetBinContent(i, (v1+v2)/2);
    if(d1==0 && d2==0)
      h1->SetBinError(i,0);
    else if(d1==0)
      h1->SetBinError(i,d2);
      ...
      h1->SetBinError(i,sqrt(d1*d1+d2*d2));
  }
}
