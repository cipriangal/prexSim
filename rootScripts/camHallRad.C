...
int camHallRad(string fnm,string simType, string metric, int det){
  ...
  if (metric == "NIEL") {
    cout<<"NEIL:\t"<<std::endl;  
    doOneDetector(h1, runFactor, det, 1);
  }
  if (metric == "Flux") {
    cout<<"NEIL:\t"<<std::endl;  
    doOneDetector(h2, runFactor, det, 1);
  }
  ...
  if (metric == "Energy") {
    cout<<"NEIL:\t"<<std::endl;  
    doOneDetector(h3, runFactor, det, 1);
  }
...
void doOneSummary(TH1D *h, double runV){
  ...
  for(int i=2;i<=nb;i+=2){
    ...
    if(det==1001 || det==1002 || det==1004 || det==1005 || det==1006 || det==1007 || det==1101 || det==1102){
